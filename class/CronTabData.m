...
-(void)increaceNumber{
    self.totalTimes = @(self.totalTimes.integerValue + 1);
    if([self.date isEqualToString:[self getCurrentDate]]){
        self.totalTimesOfToday = @(self.totalTimesOfToday.integerValue + 1);
    }else{
        self.totalTimesOfToday = @1;
        self.date = [self getCurrentDate];
        self.totalDays = @(self.totalDays.integerValue + 1);
    }
